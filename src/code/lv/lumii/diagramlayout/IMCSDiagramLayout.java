...
public class IMCSDiagramLayout {
	...
	// When adding a line or a box, we first check transposeXY, then inverseY.
	/*
	 * if (transposeXY) x<->y if (inverseY) layout.y = maxY-y-height else layout.y =
	 * minY+y layout.x += minX
	 */
	// For getLayout(), however, we must check inverseY, then transposeXY (since we
	// are obtaining the coos, not assigning).
	/*
	 * x = layout.x - minX if (inverseY) y = maxY-layout.y-height else y =
	 * layout.y-minY if (transposeXY) x<->y
	 */
	...
	private static Diagram private_createUniversalDiagram() {
		Diagram d = new Diagram(new Rectangle2D.Double(), ArrangeData.ArrangeStyle.UNIVERSAL,
				LayoutConstraints.ConstraintType.NONE);
		return d;
	}

	private static Diagram private_createHierarchicalDiagram() {
		Diagram d = new Diagram(new Rectangle2D.Double(), ArrangeData.ArrangeStyle.FLOW,
				LayoutConstraints.ConstraintType.NONE);
		return d;
	}

	private static Diagram private_createSymmetricDiagram() {
		Diagram d = new Diagram(new Rectangle2D.Double(), ArrangeData.ArrangeStyle.SPRING_EMBEDDED,
				LayoutConstraints.ConstraintType.NONE);
		return d;
	}
	...
	private static lv.lumii.layoutengine.Line.LineType getLineTypeFromString(String lineType) {
		lv.lumii.layoutengine.Line.LineType t = lv.lumii.layoutengine.Line.LineType.ORTHOGONAL;
		if ("POLYLINE".equals(lineType))
			t = lv.lumii.layoutengine.Line.LineType.POLYLINE;
		else if ("STRAIGHT".equals(lineType))
	...
	private static Line private_addLineWithPoints(Box src, Box tgt, String lineType, ArrayList<Point2D.Double> points) {
		...
		if ((first.getX() >= src.getLeft() - eps) && (first.getX() <= src.getRight() + eps)
			...
			throw new RuntimeException("Wrong line connection point: first point=" + first + "; source box="
					+ src.getLeft() + " " + src.getRight() + " " + src.getTop() + " " + src.getBottom());

		if ((last.getX() >= tgt.getLeft() - eps) && (last.getX() <= tgt.getRight() + eps)
				&& (last.getY() >= tgt.getTop() - eps) && (last.getY() <= tgt.getBottom() + eps))
			;
		else
			throw new RuntimeException("Wrong line connection point: last point=" + last + "; target box="
					+ tgt.getLeft() + " " + tgt.getRight() + " " + tgt.getTop() + " " + tgt.getBottom());
		Line l = src.connectTo(tgt, t, 5, points);
		return l;
	}

	private static lv.lumii.layoutengine.OutsideLabel.LineLabel private_addLineLabel(Line l, double w, double h,
			String placement) {
		...
		} else if ("start-right".equalsIgnoreCase(placement)) {
			p = 0;
			o = OutsideLabel.LineLabel.Orientation.CLOCKWISE;
		} else if ("end-left".equalsIgnoreCase(placement)) {
			p = 1.0;
			o = OutsideLabel.LineLabel.Orientation.COUNTERCLOCKWISE;
		} else if ("end-right".equalsIgnoreCase(placement)) {
			p = 1.0;
			o = OutsideLabel.LineLabel.Orientation.CLOCKWISE;
		} else if ("middle-left".equalsIgnoreCase(placement)) {
			o = OutsideLabel.LineLabel.Orientation.COUNTERCLOCKWISE;
		} else if ("middle-right".equalsIgnoreCase(placement)) {
			o = OutsideLabel.LineLabel.Orientation.CLOCKWISE;
		}

		return l.createLabel(w, h, p, o, lv.lumii.layoutengine.LayoutConstraints.ConstraintType.NONE, 2.0);
	}
	...
	public IMCSDiagramLayout(String arrangeStyle) {
		...
		if ((arrangeStyle != null) && ((arrangeStyle.charAt(0) == 'H') || (arrangeStyle.charAt(0) == 'V')
				|| (arrangeStyle.charAt(0) == 'I'))) {
			jDiagram = private_createHierarchicalDiagram();
			if (arrangeStyle.charAt(0) == 'H')
				this.transposeXY = true; // for HORIZONTAL or HIERARCHICAL
			if (arrangeStyle.charAt(0) == 'I') { // for INVERSE_HORIZONTAL/INVERSE_HIERARCHICAL OR INVERSE_VERTICAL
				this.inverseY = true;
				if (arrangeStyle.charAt(8) == 'H')
					this.transposeXY = true; // for INVERSE_HORIZONTAL OR INVERSE_HIERARCHICAL
			}
		} else if ((arrangeStyle != null) && (arrangeStyle.charAt(0) == 'S')) {
	...
	public boolean addLine(long lineId, long srcId, long tgtId, String lineType, int startSides, int endSides,
			ArrayList<Point2D.Double> points) {
		// adds an orthogonal line connecting the two boxes with the given ids to the
		// layout;
		// returns whether the operation succeeded;
		...
		if ((jSrc != null) && (jTgt != null)) {
			...
			if (points != null) {
				...
				/*
				 * if (!this.lastLine) if (this.transactionStarted) {
				 * this.jDiagram.endTransaction(); this.jDiagram.startTransaction(); }
				 */
	...
	public boolean addLine(double lineId, double srcId, double tgtId, String lineType, int startSides, int endSides,
			ArrayList<Point2D.Double> points) {
		return addLine((long) lineId, (long) srcId, (long) tgtId, lineType, startSides, endSides, points);
	}

	// "start-left", i.e., left, if we follow the line direction (from start to
	// end), near the start
	// "start-right",
	...
	public boolean addLineLabel(long labelId, long lineId, double w, double h, String placement) {
		...
		if (jLine != null) {
			...
			jLabel.setMinSize(w, h);
			jLabel.setRotation(false);
			jLabel.setAutoFlip(false);
	...
	public ArrayList<Long> removeLine(long lineId) {
		...
		// returns the array of id-s of the removed line labels (perhaps, an empty
		// array);
		Line jLine = this.linesMap.get(lineId);
		if (jLine != null) {
	...
	public boolean moveBox(long boxId, double newX, double newY) {
		...
		if (jBox != null) {
			...
			if (this.inverseY)
				newY = this.maxY - newY - h; // do not forget to subtract the box height
			else
				newY = this.minY + newY;
			newX += this.minX;
...
// associating the new line... 
			 ...
			 * and re-create the line... Line newLine;
			 * 
			 * if (points != null) { ArrayList<Point2D.Double> jPoints = new
			 * ArrayList<Point2D.Double>(); for (int i=0; i<points.size(); i++) {
			 * Point2D.Double p = new Point2D.Double(); p.setLocation( points.get(i) ); if
			 * (this.transposeXY) { double t=p.x; p.x=p.y; p.y=t; } if (this.inverseY) { p.y
			 * = this.maxY - p.y; } else { p.y = this.minY + p.y; } p.x = this.minX + p.x;
			 * jPoints.add(p); } newLine = ((Box) jPrevSrc).connectTo((Box) jPrevTgt,
			 * jLine.getType(), 5, points); } else newLine = ((Box)
			 * jPrevSrc).connectTo((Box) jPrevTgt, jLine.getType(), 5);
			 * 
			 * 
			 * for (int i=0; i<n; i++) { OutsideLabel.LineLabel jLabel = jLabels.get(i);
			 * 
			 * double w = jLabel.getWidth(); double h = jLabel.getHeight(); double p =
			 * jLabel.getPosition(); OutsideLabel.LineLabel.Orientation o =
			 ...
			 * 
			 * // removing the previous line (with old labels)...
			 * this.linesMap.remove(lineId); this.linesInvMap.remove(jLine);
			 * jLine.remove(false);
			 * 
			 * // associating the new line... this.linesMap.put(lineId, newLine);
			 * this.linesInvMap.put(newLine, lineId); } else
			 */
			// retrace...
			if (points != null) {
				ArrayList<Point2D.Double> jPoints = new ArrayList<Point2D.Double>();
...
//					this.moveLine(i, getStartBox(o), getEndBox(o), points);
		...
		/*
		 * for (Long i : this.boxesMap.keySet()) { Box o = this.boxesMap.get(i); try {
		 * this.moveBox(i, o.getLeft(), o.getTop()); this.jDiagram.endTransaction(); }
		 * catch(Throwable t) { } break; }
		 */

		if (retraced) {
			/*
			 * for (Long i : this.boxesMap.keySet()) { Box o = this.boxesMap.get(i);
			 * 
		...
		for (Long i : this.boxesMap.keySet()) {
			...
			Rectangle2D.Double coos = new Rectangle2D.Double(x, y, myRound(o.getWidth()), myRound(o.getHeight()));
		...
		for (Long i : this.linesMap.keySet()) {
			...
			for (int j = 0; j < n; j++) {
				...
				retVal.labels.put(labelId,
						new Rectangle2D.Double(x, y, myRound(jLabel.getWidth()), myRound(jLabel.getHeight())));
			}
		...
		for (Long i : retVal.boxes.keySet()) {
			...
			if (this.transposeXY) {
				double t = retVal.boxes.get(i).x;
				retVal.boxes.get(i).x = retVal.boxes.get(i).y;
				retVal.boxes.get(i).y = t;
				t = retVal.boxes.get(i).width;
				retVal.boxes.get(i).width = retVal.boxes.get(i).height;
				retVal.boxes.get(i).height = t;
			}
		}
		...
		for (Long i : retVal.lines.keySet()) {
			...
			for (int j = 0; j < line.size(); j++) {
				...
				if (this.transposeXY) {
					double t = line.get(j).x;
					line.get(j).x = line.get(j).y;
					line.get(j).y = t;
				}
		...
		for (Long i : retVal.labels.keySet()) {
			...
			if (this.transposeXY) {
				double t = retVal.labels.get(i).x;
				retVal.labels.get(i).x = retVal.labels.get(i).y;
				retVal.labels.get(i).y = t;
				t = retVal.labels.get(i).width;
				retVal.labels.get(i).width = retVal.labels.get(i).height;
				retVal.labels.get(i).height = t;
			}
		}
